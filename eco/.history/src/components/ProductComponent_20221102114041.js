  ...
  const renderList = products.map((product) => {
    ...
    return (
     <div className='grid grid-cols-1 lg:grid-cols-4 gap-x-4 gap-y-16  px-20 pt-12 sm:pt-20 text-black ' key={id}>
      <div className='card rounded-md w-max-[1208px] text-black  bg-black text-bold mt-[40px] '>
    <div  className=" flex justify-center items-center text-white">
    <img className="mt-3 rounded-md py-4 px-6 h-ful w-full" src={thumbnail} alt={title}/>
    </div>
    <div className="content mt-4 ">
    <button className="text-bold   bg-green-800 rounded-md text-center h-4 px-[100px]   py-[30px] text-black">{brand}</button>
  </div>
